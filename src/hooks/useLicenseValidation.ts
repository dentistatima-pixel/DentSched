...
export const useLicenseValidation = (providerId: string | null): LicenseValidationResult => {
    ...
    return useMemo(() => {
        if (!providerId) {
            return { isPrcExpired: false, isMalpracticeExpired: false, licenseAlerts: [] };
        }
        ...
        return { isPrcExpired: prcIsExpired, isMalpracticeExpired: malpracticeIsExpired, licenseAlerts: alerts };
    }, [providerId, staff]);
};