...
const ProcedureCatalog: React.FC<ProcedureCatalogProps> = ({ settings, onUpdateSettings }) => {
    ...
    const categories = ['All', ...new Set(settings.procedures.map(p => p.category))];
    const filteredProcedures = selectedCategory === 'All' 
        ? settings.procedures 
        : settings.procedures.filter(p => p.category === selectedCategory);

    return (
        <div className="animate-in fade-in duration-300">
            <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3">
                    <label className="text-xs font-black uppercase tracking-widest text-slate-500">Filter:</label>
                    <select 
                        value={selectedCategory} 
                        onChange={(e) => setSelectedCategory(e.target.value)}
                        className="p-3 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-teal-500 cursor-pointer shadow-sm"
                    >
                        {categories.map(cat => (
