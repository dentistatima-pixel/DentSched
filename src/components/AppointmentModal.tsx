...
import SmartProcedureSelect from './SmartProcedureSelect';
...
const AppointmentModal: React.FC<AppointmentModalProps> = ({
    ...
    const timeSlots = useMemo(() => {
        ...
        const slots = [];
        const start = parseInt(hours.start?.split(':')[0] || '8');
        const end = parseInt(hours.end?.split(':')[0] || '17');
            ...
            <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
                ...
                <form id="appointment-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-6">
                    <div className="relative z-50">
                        <label className="label">Patient Search *</label>
                        <input type="text" value={patientSearch} onChange={handleSearch} className="input text-lg font-bold" placeholder="Search by name, ID, or phone" disabled={readOnly || !!patientId} />
                        {searchResults.length > 0 && (
                            <div className="absolute top-full mt-2 w-full bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden">
                                {searchResults.map(p => (
                                    <div key={p.id} onClick={() => selectPatient(p)} className="p-4 hover:bg-teal-50 border-b border-slate-50 last:border-b-0 cursor-pointer transition-colors flex flex-col">
                        ...
                        {patientId && !readOnly && (
                            <button type="button" onClick={() => { setPatientId(''); setPatientSearch(''); }} className="absolute right-4 top-10 text-xs font-bold text-slate-500 hover:text-red-500 uppercase tracking-widest bg-white pl-2">
                                Clear
                            </button>
                        )}
                    </div>
