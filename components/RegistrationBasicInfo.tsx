  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-3xl border-2 border-lilac-100 shadow-sm space-y-4">
            <h4 className="text-xs font-black uppercase text-lilac-700 tracking-widest flex items-center gap-2 mb-2"><ShieldCheck size={14} className="text-lilac-600"/> Marketing & Media Consents</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label className={`flex items-start gap-4 p-4 rounded-2xl cursor-pointer border-2 transition-all ${formData.practiceCommConsent ? 'bg-teal-50 border-teal-500 shadow-md' : 'bg-white border-slate-100 opacity-60'}`}>
                    <input type="checkbox" name="practiceCommConsent" checked={formData.practiceCommConsent} onChange={handleChange} className="w-6 h-6 accent-teal-600 rounded mt-1 shrink-0" />
                    <div>
                ...
                <label className={`flex items-start gap-4 p-4 rounded-2xl cursor-pointer border-2 transition-all ${formData.clinicalMediaConsent ? 'bg-lilac-50 border-lilac-500 shadow-md' : 'bg-white border-slate-100 opacity-60'}`}>
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-600 leading-tight">I consent to the use of my clinical photos/videos for professional education and practice portfolios.</p>
                    </div>
                </label>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all cursor-pointer ${formData.isPwd ? 'bg-lilac-50 border-lilac-400 shadow-md' : 'bg-white border-slate-100 opacity-60'}`}>
                ...
                <div><div className="font-bold text-slate-800 text-sm">Patient is PWD</div><p className="text-[10px] text-slate-400 font-bold uppercase">Requires Legal Representative Access</p></div>
            </label>
            <label className={`flex items-center gap-4 p-4 rounded-2xl border-2 transition-all cursor-pointer ${formData.isSeniorDependent ? 'bg-teal-50 border-teal-400 shadow-md' : 'bg-white border-slate-100 opacity-60'}`}>
                ...
                <div><div className="font-bold text-slate-800 text-sm">Senior Dependent</div><p className="text-[10px] text-slate-400 font-bold uppercase">Managed Access Record</p></div>
            </label>
        </div>
            ...
            <div className="bg-lilac-50 border-2 border-lilac-200 p-6 rounded-3xl shadow-lg ring-4 ring-lilac-500/5 animate-in slide-in-from-top-4 duration-500">
                <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-3">
                        <div className="bg-lilac-600 text-white p-2 rounded-xl shadow-lg shadow-lilac-600/20"><Baby size={24}/></div>
                        <div>
                            <h4 className="font-black text-lilac-900 uppercase tracking-widest text-sm">Guardian & Consent Representative</h4>
                            <p className="text-[10px] text-lilac-600 font-bold uppercase mt-0.5">Verified representative details</p>
                        </div>
                    </div>
                    {!formData.guardianProfile?.linkedPatientId && (
                        <div className="relative">
                            <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-xl border border-lilac-200">
                                ...
                                <input 
                                    type="text" 
                                    placeholder="Link Registry Profile..." 
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="md:col-span-2">
                        <label className="label text-lilac-800 font-black">Representative Legal Name *</label>
                        ...
                        {formData.guardianProfile?.linkedPatientId && <div className="mt-2 flex items-center gap-1 text-[9px] font-black text-teal-600 uppercase"><CheckCircle size={10}/> Linked Registry Match (ID: {formData.guardianProfile.linkedPatientId})</div>}
                    </div>
                    <div>
                        <label className="label text-lilac-800 font-black">Authority Level *</label>
                    ...
                    <div>
                        ...
                        <input required={showGuardian} type="text" value={formData.guardianProfile?.relationship || ''} onChange={e => handleGuardianProfileChange('relationship', e.target.value)} className="input border-lilac-200 focus:border-lilac-500" placeholder="e.g. Parent, Legal Representative" />
                    </div>
                    <div>
                        <label className="label text-lilac-800 font-bold">Representative Mobile</label>
                    ...
                    <div>
                        <label className="label text-lilac-800 font-bold">Representative Email</label>
                ...
                <div className="mt-6 pt-6 border-t border-lilac-200 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-5 rounded-[2rem] border-2 border-lilac-200 shadow-sm">
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="font-black text-lilac-900 uppercase tracking-widest text-[10px]">Identity Affirmation</span>
                        </div>
                        ...
                        <textarea 
                            ...
                            placeholder="Type the affirmation above exactly as written to certify identity..."
                    ...
                    <div className="bg-white p-5 rounded-[2.5rem] border-2 border-dashed border-lilac-300 flex flex-col items-center justify-center relative overflow-hidden">
                        {!formData.biometricConsent ? (
                            <div className="text-center p-6 flex flex-col items-center gap-2">
                                ...
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-tight">Biometric Consent Required for Camera Access</p>
                            </div>
                        ) : isCameraActive ? (
                            <div className="w-full h-full flex flex-col items-center gap-3">
                                ...
                                <button type="button" onClick={captureAnchor} className="bg-lilac-600 text-white px-6 py-2 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg animate-pulse">Snap Identity Anchor</button>
                            </div>
                            ...
                            <div className="flex flex-col items-center gap-2">
                                ...
                                <div className="text-[9px] font-black text-teal-600 uppercase tracking-widest">Digital Identity Proof Captured</div>
                            ...
                            <div className="flex flex-col items-center gap-4 text-center">
                                ...
                                <div>
                                    <h5 className="font-black text-lilac-900 uppercase text-[10px] tracking-widest">Visual Identity Proof</h5>
                                    <p className="text-[10px] text-slate-400 mt-1 max-w-[150px]">Capture of representative image for registry integrity.</p>
                                </div>
                                <button type="button" disabled={cameraLoading} onClick={startCamera} className="px-6 py-2.5 bg-lilac-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg hover:scale-105 transition-all">
                                    {cameraLoading ? 'Initializing Lens...' : 'Capture Proof'}
                                </button>
                            </div>
                        )}
                        <canvas ref={canvasRef} className="hidden" />
                        {formData.guardianProfile?.visualAnchorHash && (
                            <div className="absolute bottom-2 left-0 right-0 text-center"><span className="text-[8px] font-mono text-lilac-300">INTEGRITY_HASH: {formData.guardianProfile.visualAnchorHash.substring(0, 16)}...</span></div>
                        )}
                    </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-lilac-100">
                    <label className={`flex items-start gap-4 p-4 rounded-2xl border-2 transition-all cursor-pointer ${formData.biometricConsent ? 'bg-teal-50 border-teal-500 shadow-md' : 'bg-white border-slate-100'}`}>
                        <input type="checkbox" name="biometricConsent" checked={formData.biometricConsent} onChange={handleChange} className="w-6 h-6 accent-teal-600 rounded mt-1 shrink-0" />
                        <div>
                            <span className="font-extrabold text-teal-900 uppercase text-[10px]">Explicit Biometric & Image Consent *</span>
                            <p className="text-[10px] text-slate-600 font-medium leading-relaxed mt-1">I expressly consent to the capture of my visual image for forensic verification purposes in this patient registry. I understand this data is stored strictly for legal identity validation.</p>
                        </div>
                    </label>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
                <h4 className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2 mb-2"><Phone size={14}/> Primary Contact</h4>
                <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2 md:col-span-1"><label className="label font-bold text-teal-800">Mobile Number *</label><input required type="tel" name="phone" value={formData.phone || ''} onChange={handleChange} className="input" placeholder="09XX-XXX-XXXX" /></div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
                <h4 className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2 mb-2"><Briefcase size={14}/> Professional Details</h4>
                <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white p-6 rounded-3xl border-2 border-teal-50 shadow-sm space-y-4">
            <h4 className="text-xs font-black uppercase text-teal-700 tracking-widest flex items-center gap-2 mb-2"><Users size={14} className="text-teal-600"/> Family & Social Context</h4>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="md:col-span-1"><label className="label">Father's Legal Name</label><input type="text" name="fatherName" value={formData.fatherName || ''} onChange={handleChange} className="input" placeholder="Full legal name" /></div>
                ...
                <div className="md:col-span-1"><label className="label">Mother's Legal Name</label><input type="text" name="motherName" value={formData.motherName || ''} onChange={handleChange} className="input" placeholder="Full legal name" /></div>
            ...
            <div className="pt-4 border-t border-slate-100">
                <label className={`flex items-start gap-4 p-4 rounded-2xl cursor-pointer border-2 transition-all ${formData.thirdPartyAttestation ? 'bg-teal-50 border-teal-500 shadow-md' : 'bg-white border-slate-100'}`}>
                    <input type="checkbox" name="thirdPartyAttestation" checked={formData.thirdPartyAttestation} onChange={handleChange} className="w-6 h-6 accent-teal-600 rounded mt-1 shrink-0" />
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <ShieldAlert size={14} className="text-teal-600"/>
                            <span className="font-extrabold text-teal-900 uppercase text-[10px]">Data Provision Attestation *</span>
                        </div>
                        <p className="text-[10px] text-slate-600 font-medium leading-relaxed">I certify that I have the necessary authority from the family members listed above to provide their information to this clinic for coordination of care.</p>
                    </div>
                </label>
            </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
            <h4 className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2 mb-2"><CreditCard size={14} className="text-teal-600"/> Insurance & Coverage</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="label">HMO / Insurance Provider</label><select name="insuranceProvider" value={formData.insuranceProvider || ''} onChange={handleChange} className="input">{['', ...fieldSettings.insuranceProviders].map(ins => <option key={ins} value={ins}>{ins || '- Private Pay -'}</option>)}</select></div>
                <div><label className="label">Insurance Policy Number</label><input type="text" name="insuranceNumber" value={formData.insuranceNumber || ''} onChange={handleChange} className="input" placeholder="e.g. 123-456-789" /></div>
            </div>
        </div>
    </div>
  );
