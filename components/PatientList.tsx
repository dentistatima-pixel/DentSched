...
const PatientList: React.FC<PatientListProps> = ({ 
  ...
  // DPA HARD BLOCK LOGIC
  const isSuspended = selectedPatient?.processingStatus === 'Suspended';

  const handleSignOffToday = async () => {
      if (!selectedPatient) return;
      if (isSuspended) { toast.error("HARD BLOCK: Cannot sign records while processing is suspended."); return; }
  ...
  const handleQuickUpdateChart = (entry: DentalChartEntry) => {
      ...
      if (isSuspended) { toast.error("HARD BLOCK: Cannot update records while processing is suspended."); return; }
  ...
  const handleAddChartEntry = async (newEntry: DentalChartEntry) => {
      if (!selectedPatient || isClinicalReadOnly) return;
      if (isSuspended) { toast.error("HARD BLOCK: Cannot add entries while processing is suspended."); return; }
  ...
  const toggleMarketingConsent = async () => {
      if (!selectedPatient) return;
      if (isSuspended) { toast.error("HARD BLOCK: Cannot modify consent while processing is suspended."); return; }
    ...
    <div className="h-full flex flex-col md:flex-row gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      ...
      {selectedPatient ? (
        <div className="flex-[2] bg-white rounded-2xl shadow-sm border border-slate-100 p-0 relative animate-in slide-in-from-right-10 duration-300 overflow-hidden flex flex-col">
           {isSuspended && (
               <div className="bg-red-600 text-white px-6 py-2 flex items-center justify-center gap-2 font-bold text-xs uppercase tracking-widest animate-pulse z-20">
                   <ShieldAlert size={16}/> Data Processing Suspended: Record Locked for Audit
               </div>
           )}
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2 mt-4 flex-wrap">
                        ...
                        <button 
                            ...
                            className="p-3 rounded-xl font-medium flex items-center justify-center transition-colors bg-lilac-100 hover:bg-lilac-200 text-lilac-700 disabled:opacity-30 disabled:grayscale" 
                            title="Book"
                        >
                        ...
                        <button 
                            ...
                            className="p-3 rounded-xl font-medium flex items-center justify-center transition-colors bg-slate-100 hover:bg-slate-200 text-slate-700 disabled:opacity-30 disabled:grayscale" 
                            title="Edit"
                        >
                            <Pencil size={20} />
                        </button>
                        {canDelete && !selectedPatient.isArchived && (
                            <button 
                                ...
                                className="p-3 rounded-xl font-medium bg-slate-100 hover:bg-red-100 text-slate-600 hover:text-red-600 ml-auto disabled:opacity-30 disabled:grayscale" 
                                title="Archive"
                            >
                                <Archive size={20} />
                            </button>
           ...
           <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-slate-50/50">
                     ...
                     <div className="space-y-6">
                         <div className="flex items-center justify-between">
                            ...
                            {(currentUser.role === UserRole.DENTIST || currentUser.role === UserRole.ADMIN) && (
                                <button 
                                    disabled={isSuspended}
                                    onClick={handleSignOffToday} 
                                    className="px-4 py-2 bg-lilac-600 text-white text-xs font-bold rounded-xl flex items-center gap-2 shadow-lg hover:bg-lilac-700 transition-all disabled:opacity-50 disabled:grayscale"
                                >
                                    <ShieldCheck size={16}/> Sign-Off Today
                                </button>
                            )}
                         </div>
                         <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden"><Odontonotes entries={selectedPatient.dentalChart || []} onAddEntry={handleAddChartEntry} onUpdateEntry={handleQuickUpdateChart} currentUser={currentUser.name} readOnly={isClinicalReadOnly || isSuspended} procedures={fieldSettings?.procedures || []}/></div>
                     </div>
                )}
                ...
                {activeTab === 'ledger' && (<PatientLedger patient={selectedPatient} onUpdatePatient={onQuickUpdatePatient} readOnly={isClinicalReadOnly && !currentUser.canViewFinancials || isSuspended} fieldSettings={fieldSettings} />)}
                {activeTab === 'documents' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <button 
                            ...
                            className="p-6 bg-white border-2 border-slate-100 rounded-2xl flex items-center gap-4 hover:border-teal-500 hover:shadow-xl transition-all group disabled:opacity-50 disabled:grayscale"
                        >
                        ...
                        <button 
                            disabled={isSuspended}
                            onClick={() => setIsEPrescriptionModalOpen(true)} 
                            className="p-6 bg-white border-2 border-slate-100 rounded-2xl flex items-center gap-4 hover:border-teal-500 hover:shadow-xl transition-all group disabled:opacity-50 disabled:grayscale"
                        >
                ...
                {activeTab === 'perio' && (<PerioChart data={selectedPatient.perioChart || []} onSave={(d) => onQuickUpdatePatient({...selectedPatient, perioChart: d})} readOnly={isClinicalReadOnly || isSuspended}/>)}
                {activeTab === 'plan' && (<TreatmentPlan patient={selectedPatient} onUpdatePatient={onQuickUpdatePatient} currentUser={currentUser} logAction={logAction} featureFlags={fieldSettings?.features} readOnly={isSuspended}/>)}
           </div>
        </div>
