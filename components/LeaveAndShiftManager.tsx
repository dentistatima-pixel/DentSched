...
import { Calendar, Check, X, Plus, Send, ShieldCheck, Sun, Moon, Briefcase, UserX, UserCheck, ArrowLeft, Phone, User as UserIcon, MapPin } from 'lucide-react';
...
interface LeaveAndShiftManagerProps {
    ...
    initialTab?: 'requests' | 'roster';
}

const LeaveAndShiftManager: React.FC<LeaveAndShiftManagerProps> = ({ staff, currentUser, leaveRequests, onAddLeaveRequest, onApproveLeaveRequest, fieldSettings, onBack, onUpdateStaffRoster, initialTab }) => {
    ...
    return (
        <div className="h-full flex flex-col gap-6 animate-in fade-in duration-500 p-8">
            ...
            <div className="bg-white p-2 rounded-2xl border border-slate-100 shadow-sm self-start flex gap-2">
                <button onClick={() => setActiveTab('requests')} className={`flex items-center gap-2 px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${activeTab === 'requests' ? 'bg-teal-900 text-white shadow-lg' : 'bg-white text-slate-400'}`}>
                    Leave Requests
                </button>
                <button onClick={() => setActiveTab('roster')} className={`flex items-center gap-2 px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest ${activeTab === 'roster' ? 'bg-teal-900 text-white shadow-lg' : 'bg-white text-slate-400'}`}>
                    Weekly Roster
                </button>
            </div>
                    ...
                    <div className="space-y-3">
                        {leaveRequests.filter(r => r.status === 'Pending').map(req => (
                            <div key={req.id} className="bg-slate-50 p-5 rounded-2xl border border-slate-100 flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className="bg-white p-3 rounded-lg shadow-sm text-slate-500"><UserIcon size={24} /></div>
                                    <div>
