...
const PracticeBranding: React.FC<PracticeBrandingProps> = ({ settings, onUpdateSettings }) => {
    ...
    const tabs = [
        { id: 'identity', label: 'Theme & Profile', icon: Sparkles },
    ...
    return (
        <div className="p-10 space-y-8 animate-in fade-in duration-500">
            <div>
                <h3 className="text-3xl font-black text-slate-800 uppercase tracking-tighter leading-none">Practice Identity Hub</h3>
                <p className="text-sm text-slate-500 font-bold uppercase tracking-widest mt-2">Manage global branding, locations, and network affiliations.</p>
            </div>
            ...
            {activeTab === 'identity' && (
                 <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-8">
                    ...
                    <div className="flex justify-between items-center bg-slate-50 p-6 rounded-2xl border border-slate-100">
                        <div><h4 className="font-bold text-slate-800">Interface Theme</h4><p className="text-sm text-slate-500">Current: <span className="font-bold capitalize">{theme}</span></p></div>
                        <div className="flex bg-slate-200 p-1.5 rounded-full border border-slate-300">
                            <button onClick={() => theme === 'dark' && toggleTheme()} className={`px-6 py-2 rounded-full text-xs font-black uppercase flex items-center gap-2 ${theme === 'light' ? 'bg-white text-teal-800 shadow-md' : 'text-slate-500'}`}><Sun size={14}/> Light</button>
                            <button onClick={() => theme === 'light' && toggleTheme()} className={`px-6 py-2 rounded-full text-xs font-black uppercase flex items-center gap-2 ${theme === 'dark' ? 'bg-slate-800 text-white shadow-md' : 'text-slate-500'}`}><Moon size={14}/> Dark</button>
                        </div>
                    </div>
                </div>
            )}
                ...
                <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-100">
                        {settings.branchProfiles.map(b => (
                            <div key={b.id} className="p-4 bg-slate-50 rounded-2xl flex justify-between items-center border border-slate-100 group">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-lg bg-teal-100 flex items-center justify-center"><MapPin className="text-teal-600"/></div>
                                    <div>
                                        <div className="font-black text-slate-800 uppercase tracking-tight">{b.name}</div>
                                        <div className="text-xs font-bold text-slate-500">{b.address}</div>
                                    </div>
                                </div>
                                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button onClick={() => setEditingBranch(b)} className="p-2 bg-white rounded-lg text-slate-500 hover:text-blue-600"><Edit size={16}/></button>
                                    <button onClick={() => handleRemoveBranch(b.id)} className="p-2 bg-white rounded-lg text-slate-500 hover:text-red-600"><Trash2 size={16}/></button>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-6">
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-100">
                        {settings.hospitalAffiliations.map(h => (
                             <div key={h.id} className="p-4 bg-slate-50 rounded-2xl flex justify-between items-center border border-slate-100 group">
                                <div>
                                    <div className="font-black text-slate-800 uppercase tracking-tight">{h.name}</div>
                                    <div className="text-xs font-bold text-slate-500">{h.location} &bull; {h.hotline}</div>
                                </div>
                                <button onClick={() => handleRemoveAffiliation(h.id)} className="text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={18}/></button>
                            </div>
                        ))}
                    </div>
                </div>
