...
import { DollarSign, Plus, ArrowUpRight, ArrowDownLeft, Receipt, Calendar, CreditCard, Heart, Ban, AlertCircle, Shield, Tag, Hash, FileCheck, CheckCircle2 } from 'lucide-react';
...
const PatientLedger: React.FC<PatientLedgerProps> = ({ patient, onUpdatePatient, readOnly, onPreparePhilHealthClaim, onCreateClaim, fieldSettings }) => {
    ...
    const handlePostPending = (entry: DentalChartEntry) => {
        if (readOnly) return;
        
        const newBalance = currentBalance + (entry.price || 0);
        const ledgerEntry: LedgerEntry = {
            id: `l_${Date.now()}_${entry.id}`,
            date: entry.date || new Date().toISOString().split('T')[0],
            description: `${entry.procedure} ${entry.toothNumber ? `(#${entry.toothNumber})` : ''} - From Clinical Record`,
            type: 'Charge',
            amount: entry.price || 0,
            balanceAfter: newBalance,
            notes: `Auto-posted from Chart ID: ${entry.id}`
        };

        const updatedChart = patient.dentalChart?.map(e => e.id === entry.id ? { ...e, isBilled: true } : e);
        ...
        toast.success(`Procedure posted to ledger.`);
    };
    ...
    return (
        <div className="h-full flex flex-col bg-slate-50 rounded-xl border border-slate-200 overflow-hidden relative">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                ...
                {!readOnly && pendingProcedures.length > 0 && (
                    <div className="animate-in slide-in-from-top-4 duration-500">
                        <div className="flex items-center gap-2 text-lilac-600 font-bold text-xs uppercase mb-2 ml-1 tracking-widest">
                            <AlertCircle size={14} /> Pending Charges from Clinical Record
                        </div>
                        <div className="bg-lilac-50 border-2 border-lilac-100 rounded-2xl overflow-hidden shadow-sm">
                            <div className="divide-y divide-lilac-100">
                                {pendingProcedures.map(entry => (
                                    <div key={entry.id} className="p-4 flex items-center justify-between hover:bg-lilac-100/50 transition-colors">
                                        <div className="flex items-center gap-4">
                                            <div className="bg-white p-2 rounded-lg border border-lilac-200 font-mono text-xs font-bold text-lilac-700">#{entry.toothNumber || '0'}</div>
                                            <div>
                                                <div className="text-sm font-bold text-slate-800">{entry.procedure}</div>
                                                <div className="text-[10px] text-slate-500 font-bold uppercase">Logged by Dr. {entry.author} on {formatDate(entry.date)}</div>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-6">
                                            <div className="text-right">
                                                <div className="text-sm font-extrabold text-slate-800">₱{entry.price?.toLocaleString()}</div>
                                                <div className="text-[9px] text-lilac-600 font-bold uppercase">Clinical Fee</div>
                                            </div>
                                            <button 
                                                onClick={() => handlePostPending(entry)}
                                                className="bg-lilac-600 hover:bg-lilac-700 text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 shadow-lg shadow-lilac-600/20 transition-all active:scale-95"
                                            >
                ...
                {mode !== 'view' && (
                    <div className="p-0 bg-slate-100 border-b animate-in slide-in-from-top-2">
                        <form onSubmit={handleTransaction} className="bg-white p-6 rounded-2xl shadow-xl border border-slate-200 space-y-4">
                ...
                <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
                            ...
                            {[...ledger].reverse().map((entry) => (
                                <tr key={entry.id} className="hover:bg-slate-50 group transition-colors">
                                    ...
                                    <td className="p-4 font-medium">
                                        {entry.description}
                                        {entry.orNumber && <span className="ml-2 font-mono text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded font-bold border border-slate-200">OR: {entry.orNumber}</span>}
                                        {entry.discountType && <span className="ml-2 bg-amber-100 text-amber-700 text-[9px] px-1.5 py-0.5 rounded font-bold border border-amber-200">{entry.discountType}</span>}
                                    </td>
                    ...
                    {ledger.length === 0 && (
                        <div className="p-20 text-center text-slate-400 italic">
                            No financial transactions on record.
                        </div>
                    )}
                </div>
            </div>
