...
const PatientLedger: React.FC<PatientLedgerProps> = ({ patient, onUpdatePatient, readOnly, fieldSettings }) => {
    ...
    const [amount, setAmount] = useState(''); // For simple payments
    ...
    const handleCharge = (e: React.FormEvent) => {
        ...
        const mat = parseFloat(matAmount) || 0;
        const total = pf + mat;
        if (total <= 0) return;

        const newBalance = currentBalance + total;
        const newEntry: LedgerEntry = {
            id: Math.random().toString(36).substr(2, 9), date, description, type: 'Charge', amount: total, pfAmount: pf, materialAmount: mat, isVatExempt, balanceAfter: newBalance
        };

        onUpdatePatient({ ...patient, ledger: [...ledger, newEntry], currentBalance: newBalance });
        setMode('view'); setPfAmount(''); setMatAmount(''); setDescription(''); setIsVatExempt(false);
        toast.success("Split charge recorded.");
    };
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
                {mode === 'add_charge' && (
                    <form onSubmit={handleCharge} className="bg-white p-6 rounded-2xl border-2 border-teal-500 space-y-4 animate-in zoom-in-95 shadow-xl">
                        <div className="flex justify-between items-center mb-2">
                            <h4 className="font-black text-teal-900 flex items-center gap-2 uppercase tracking-widest text-sm"><Layers size={20}/> Tiered Clinical Charge (PF vs Materials)</h4>
                            <label className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border transition-all cursor-pointer ${isVatExempt ? 'bg-amber-50 border-amber-300 text-amber-800' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
                                <input type="checkbox" checked={isVatExempt} onChange={e => setIsVatExempt(e.target.checked)} className="w-4 h-4 accent-amber-600" />
                                <span className="text-[10px] font-black uppercase">Senior/PWD VAT-Exempt</span>
                            </label>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="md:col-span-3"><label className="text-[10px] font-bold text-slate-500 uppercase ml-1">Procedure Description</label><input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g. Surgical Extraction #16" className="w-full p-3 rounded-xl border-slate-200 focus:border-teal-500 outline-none" required /></div>
                            <div><label className="text-[10px] font-bold text-slate-500 uppercase ml-1">Prof. Fee (₱)</label><input type="number" value={pfAmount} onChange={e => setPfAmount(e.target.value)} placeholder="0.00" className="w-full p-3 rounded-xl border-slate-200 focus:border-teal-500 outline-none font-bold" /></div>
                            <div><label className="text-[10px] font-bold text-slate-500 uppercase ml-1">Materials (₱)</label><input type="number" value={matAmount} onChange={e => setMatAmount(e.target.value)} placeholder="0.00" className="w-full p-3 rounded-xl border-slate-200 focus:border-teal-500 outline-none font-bold" /></div>
                            <div><label className="text-[10px] font-bold text-slate-500 uppercase ml-1">Date</label><input type="date" value={date} onChange={e => setDate(e.target.value)} className="w-full p-3 rounded-xl border-slate-200 outline-none" /></div>
                        </div>
                        <div className="bg-slate-50 p-4 rounded-xl flex justify-between items-center border border-dashed border-slate-200">
                             <div className="text-sm font-bold text-slate-500 uppercase">Total Charge:</div>
                             <div className="text-2xl font-black text-teal-800">₱{((parseFloat(pfAmount)||0) + (parseFloat(matAmount)||0)).toLocaleString()}</div>
                        </div>
                ...
                {mode === 'add_payment' && (
                    <form onSubmit={handlePayment} className="bg-white p-6 rounded-2xl border-2 border-emerald-500 space-y-4 animate-in zoom-in-95 shadow-xl">
                        <h4 className="font-bold text-emerald-900 flex items-center gap-2"><DollarSign size={20}/> Receive Payment</h4>
                            ...
                            {[...ledger].reverse().map((entry) => (
                                <tr key={entry.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="p-4">
                                        ...
                                        {entry.pfAmount !== undefined && (
                                            <div className="flex gap-2 mt-0.5">
                                                <span className="text-[9px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded uppercase font-bold">PF: ₱{entry.pfAmount.toLocaleString()}</span>
                                                <span className="text-[9px] bg-slate-50 text-slate-500 px-1.5 py-0.5 rounded uppercase font-bold">Mat: ₱{entry.materialAmount?.toLocaleString()}</span>
                                                {entry.isVatExempt && <span className="text-[9px] bg-amber-50 text-amber-700 px-1.5 py-0.5 rounded uppercase font-bold border border-amber-100">VAT-EXEMPT</span>}
                                            </div>
                                        )}
                                    </td>
