...
const TreatmentPlan: React.FC<TreatmentPlanProps> = ({ patient, onUpdatePatient, readOnly, currentUser, logAction, featureFlags, fieldSettings, onOpenRevocationModal }) => {
    ...
    return (
        <div className="h-full flex flex-col space-y-8 pb-20 animate-in fade-in duration-500">
            ...
            {varianceModal && (
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex justify-center items-center p-4 animate-in fade-in">
                    <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl p-10 space-y-8 animate-in zoom-in-95 border-4 border-amber-100">
                        <div className="flex items-center gap-5">
                            <div className="bg-amber-100 p-4 rounded-3xl text-amber-700 shadow-sm"><Receipt size={32}/></div>
                            <div><h3 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Financial Variance</h3><p className="text-[10px] text-amber-600 font-black uppercase tracking-widest mt-1">Rule 16 Quote Deviation Log</p></div>
                        </div>
                        <div className="space-y-6">
                            <div className="bg-slate-50 p-6 rounded-3xl border-2 border-slate-100 shadow-inner grid grid-cols-2 gap-6">
                                <div><div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Original Quote</div><div className="text-2xl font-black text-slate-700">₱{varianceModal.quote.toLocaleString()}</div></div>
                                <div><div className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1">Actual Charge</div><div className="text-2xl font-black text-amber-800">₱{varianceModal.actual.toLocaleString()}</div></div>
                            </div>
                            <div className="animate-in slide-in-from-top-2">
                                <label className="label text-[10px] text-amber-700">Clinical Narrative for Variance *</label>
                                <textarea value={varianceNarrative} onChange={e => setVarianceNarrative(e.target.value)} className="input h-32 focus:border-amber-500" placeholder="e.g. Additional material was required due to unforeseen clinical complexity..." />
                            </div>
                        </div>
                        <div className="flex gap-3 pt-4"><button onClick={() => setVarianceModal(null)} className="flex-1 py-4 font-black uppercase text-[10px] text-slate-400">Cancel</button><button onClick={handleVarianceSubmit} className="flex-[2] py-5 bg-amber-600 text-white rounded-3xl font-black uppercase text-[10px] shadow-xl shadow-amber-600/30">Finalize & Log Variance</button></div>
                    </div>
                </div>
            )}
        </div>
