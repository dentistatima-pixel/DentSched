...
export const TreatmentPlan: React.FC<TreatmentPlanProps> = ({ patient, onUpdatePatient, readOnly, currentUser, logAction, featureFlags, fieldSettings, onOpenRevocationModal }) => {
        ...
        <div className="h-full flex flex-col space-y-8 pb-20 animate-in fade-in duration-500">
            ...
            <div className="flex justify-between items-center px-4">
                <div className="flex items-center gap-4">
                    <div className="bg-lilac-100 p-3 rounded-2xl text-lilac-700 shadow-sm"><ClipboardList size={24}/></div>
                    <h3 className="font-black text-slate-800 uppercase tracking-tighter text-xl">Strategic Treatment Plans</h3>
                </div>
                {!isCreating && (
                    <button onClick={() => setIsCreating(true)} className="px-6 py-2.5 bg-lilac-600 text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-xl shadow-lilac-600/20 hover:scale-105 transition-all flex items-center gap-2">
            ...
            {isCreating && (
                <div className="bg-white p-6 rounded-2xl border-2 border-dashed border-lilac-400 flex items-center gap-4 animate-in zoom-in-95">
                    <input autoFocus type="text" value={newPlanName} onChange={e => setNewPlanName(e.target.value)} placeholder="Enter plan name (e.g., Phase I - Restorative)" className="flex-1 input" />
                    <button onClick={handleCreatePlan} className="px-5 py-2 bg-teal-600 text-white rounded-xl text-xs font-black">Save</button>
                    <button onClick={() => setIsCreating(false)}><X size={18} className="text-slate-400"/></button>
                </div>
            )}
            ...
            <div className="space-y-6">
                {allPlans.map(plan => {
                    ...
                    const progress = planItems.length > 0 ? (completedItems / planItems.length) * 100 : 0;
                    const planTotal = planItems.reduce((acc, item) => acc + (item.price || 0), 0);
                    
                    const canApprove = plan.status === TreatmentPlanStatus.PENDING_REVIEW && (currentUser.role === UserRole.DENTIST || currentUser.role === UserRole.ADMIN);
                    
                    return (
                        <div key={plan.id} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden group">
                           {/* ... JSX for rendering a single plan */}
                        </div>
                    );
                })}
            </div>
            ...
            {consentPlan && (
                <FinancialConsentModal 
                    ...
                    patient={patient}
                    plan={consentPlan}
                    planItems={(patient.dentalChart || []).filter(item => item.planId === consentPlan.id)}
                    onSave={(signature) => {
                        const updatedPlans = allPlans.map(p => p.id === consentPlan.id ? { ...p, status: TreatmentPlanStatus.APPROVED, financialConsentSignature: signature } : p);
                        onUpdatePatient({ ...patient, treatmentPlans: updatedPlans });
                        setConsentPlan(null);
                        toast.success("Financial consent digitally sealed.");
                    }}
                />
            )}
            {/* Other modals */}
